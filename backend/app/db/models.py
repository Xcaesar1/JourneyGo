...
class Trip(Base):
    ...
    tasks: Mapped[list[TripTask]] = relationship(
...
class TripTask(Base):
    ...
    __table_args__ = (Index("ix_trip_tasks_status_updated_at", "status", "updated_at"),)
    ...
    pending_input: Mapped[dict[str, Any] | None] = mapped_column(JSON)
...
class TravelQuery(Base):
    """An immutable dispatch intent; ambiguous attempts are never automatically resent."""
    ...
    result: Mapped[dict[str, Any] | list | None] = mapped_column(JSON)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
    finished_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class SourceEvidenceRecord(Base):
    ...
    __table_args__ = (Index("ix_source_evidence_source_bucket", "source_bucket"),)
