...
- 后续样式调整：首页“回忆”入口改为直角矩形，最小宽度 64px、触控高度 44px；导航位置和功能不变。此样式调整尚未部署。
- 首页移除最近保存列表，顶部 GitHub 与语言切换之间增加“回忆”入口；新页面 `/history` 按需加载，复用现有接口展示最近 50 条行程。
- 手机单列、桌面双列卡片；提供刷新、空状态、错误重试及详情返回，摘要限制两行，不展示技术编号。
- 首页草稿、景点选择和筛选在本次页面往返中保留；只存内存，刷新浏览器不保留。返回列表恢复位置，刷新失败保留已有记录。
- 打开历史行程时清理前一条结果的任务和审核缓存，防止恢复错误行程；四语言文案同步。
- 本地验证：31 项前端测试、类型检查及构建通过；后端测试通过（4 项跳过）。模拟 API 的 Chrome 回归覆盖四语言、360/390/1440 宽度、0/1/50 条记录、错误重试、草稿/景点选择及列表位置恢复；现有首页和结果页/PNG 导出回归通过。
- 生产构建的本地 gzip 检查：主脚本 955933 解码字节、309881 传输字节，未提前加载回忆/结果页或请求历史接口；非真实手机或线上性能承诺。构建保留既有大包和旧资源路径警告。
- 无后端接口、数据库或权限变更；未推送或部署。回滚本次前端提交并重新构建即可恢复首页列表，无数据回滚。

## Selective UI Loading - 2026-09-17

- 用首页所需 Ant Design 组件注册替换全量插件，结果页独有组件局部导入；保留现有 UI 和功能。
- html2canvas 延后到实际导出时加载，结果页菜单事件兼容组件声明的字符串/数字键类型。
- 首页主脚本由上一轮 1754898 字节降至 955448 字节，gzip 实际传输 309646 字节；相对最初约 3.19 MB 减少约 70%。
- 本地模拟 1.6 Mbps / 150 ms 延迟，单次冷启动按钮约 2777 ms 可用，非真实手机线上速度承诺。
- 30 项前端测试、构建、四语言首页、日期选择、结果页和实际 PNG 下载回归通过；无 API/数据库变更，回滚本提交恢复此前组件加载方式。未推送或部署。
